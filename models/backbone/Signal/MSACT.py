...
def _seq2grid(x: Tensor, region_h: int, region_w: int, region_size: Tuple[int]):
    ...
    x = torch.einsum('bmhwpqd->bmdhpwq', x).reshape(bs, nhead * head_dim, region_h * region_size[0], region_w * region_size[1])
    return x
...
def regional_routing_attention_torch(query: Tensor, key: Tensor, value: Tensor, scale: float,
                                     region_graph: LongTensor, region_size: Tuple[int],
                                     kv_region_size: Optional[Tuple[int]] = None, auto_pad=True) -> Tensor:
    ...
    key_g = torch.gather(key.view(bs, nhead, 1, kv_nregion, kv_region_size, head_dim).expand(-1, -1, query.size(2), -1, -1, -1),
                         dim=3, index=broadcasted_region_graph)
    value_g = torch.gather(value.view(bs, nhead, 1, kv_nregion, kv_region_size, head_dim).expand(-1, -1, query.size(2), -1, -1, -1),
                           dim=3, index=broadcasted_region_graph)
...
class DSATBlock(nn.Module):
    ...
    def forward(self, x, ret_attn_mask=False):
        ...
        x_r = x.view(-1, C, H, W)
        if ret_attn_mask:
            x, attn_mat = self.BRA(x_r, ret_attn_mask=True)
            B, C, H, W = x.size()
            x = x.view(B, C, N)
            x = self.drop_path(x)
            x = x + x1
...
class MCSAT(nn.Module):
    ...
    def forward(self, x, ret_attn_mask=False):
        ...
        x = x.transpose(-1, -2)
        if ret_attn_mask:
            x, attn_mat = self.block(x, ret_attn_mask=True)
